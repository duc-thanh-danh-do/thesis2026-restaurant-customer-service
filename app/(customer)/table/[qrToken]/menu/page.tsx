...
export default function MenuPage() {
  ...
  const categories = useMemo(() => Array.from(new Set(MENU_ITEMS.map((item) => item.category))), []);
  ...
  return (
    <CustomerMobileLayout>
        ...
        <div className="w-full space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {filteredItems.map((item) => (
              <article key={item.id} className="flex gap-4 rounded-[20px] border border-[#d5e1ec] bg-white p-4">
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-lg bg-[#f5f9fc]">
                  <svg className="h-8 w-8 text-[#438ed8]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="mb-1 flex items-start justify-between gap-3">
                    <h3 className="font-semibold text-[#142653]">{item.name}</h3>
                    <span className="shrink-0 text-sm font-medium text-[#142653]">EUR {item.price.toFixed(2)}</span>
                  </div>
                  <p className="mb-3 text-sm leading-relaxed text-gray-500">{item.description}</p>
                  <div className="flex flex-wrap gap-1">
                    {[
                      ...item.tags.map((tag) => ({ kind: 'tag', label: tag })),
                      ...item.allergens.map((allergen) => ({ kind: 'allergen', label: allergen })),
                    ].map((chip) => (
                      <span key={`${chip.kind}-${chip.label}`} className="rounded-full bg-[#f5f9fc] px-2 py-0.5 text-[10px] font-medium text-[#438ed8]">
                        {chip.label}
                      </span>
                    ))}
                  </div>
      ...
      <CustomerBottomNav activeTab="menu" basePath={basePath} cartCount={cartCount > 0 ? cartCount : undefined} orderCount={1} />
    </CustomerMobileLayout>
  );
}
