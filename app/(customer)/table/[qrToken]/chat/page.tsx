...
export default function ChatPage() {
  ...
  const sendMessage = (content = message) => {
    ...
    setMessages((current) => [
      ...current,
      { id: `user-${now.getTime()}`, sender: 'user', content: trimmedMessage, timestamp: now },
      {
        id: `assistant-${now.getTime()}`,
        sender: 'assistant',
        content: 'I can help with that. A staff member can also see your request if it needs service at the table.',
        timestamp: now,
      },
    ]);
    setMessage('');
    ...
    <CustomerMobileLayout>
      ...
      <div className="flex items-center justify-between gap-3 border-b border-[#d5e1ec] bg-white px-4 py-3">
        <div className="min-w-0">
          <span className="text-sm font-medium text-[#142653]">Request bill</span>
          ...
          {messages.map((msg) => (
            <div key={msg.id} className={`flex ${msg.sender === 'assistant' ? 'justify-start' : 'justify-end'}`}>
              <div className={`flex max-w-[88%] items-start gap-2 ${msg.sender === 'user' ? 'flex-row-reverse' : ''}`}>
                ...
                >
                  {msg.sender === 'assistant' ? <Bot className="h-4 w-4 text-white" /> : <span className="text-xs font-bold text-white">You</span>}
                </div>

                <div
                  className={`rounded-2xl px-4 py-3 ${
                    msg.sender === 'assistant' ? 'border border-[#d5e1ec] bg-white' : 'bg-[#142653] text-white'
                  }`}
                >
                  <p className="text-sm">{msg.content}</p>
                  <p className={`mt-1 text-xs ${msg.sender === 'assistant' ? 'text-gray-500' : 'text-white/70'}`}>
      ...
      <div className="border-t border-[#d5e1ec] bg-white p-4">
        <div className="w-full">
          <div className="mb-3 flex gap-2 overflow-x-auto">
            <button
              onClick={() => sendMessage('Which dishes are safe for my allergens?')}
      ...
      <CustomerBottomNav activeTab="chat" basePath={basePath} cartCount={cartCount > 0 ? cartCount : undefined} orderCount={1} />
    </CustomerMobileLayout>
  );
}
